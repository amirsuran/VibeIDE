 ...
 * in the React composer as `vibeide.chat.simplifiedControls`.
 */
...
// FA6 solid: `sliders` (many knobs → full view) / `minus` (minimal → simplified view). Cycling icons,
// NOT a switch: the Command Center renders the action's icon and swaps it on the `toggled` condition.
...
Registry.as<IConfigurationRegistry>(ConfigurationExtensions.Configuration).registerConfiguration({
	...
	properties: {
		[CONFIG_SIMPLIFIED_CONTROLS]: {
			type: 'boolean',
			default: false,
			description: localize('vibeide.chat.simplifiedControlsDescription', 'Упрощённый вид панели чата: под полем ввода видны только «Режим» и «Модель», остальные настройки скрыты (сохраняя значения), а автопилот и рекурсия по ссылкам принудительно включены. Переключается иконкой в шапке рядом с тумблером неона.'),
...
registerAction2(class VibeideToggleSimplifiedControls extends Action2 {
	...
	constructor() {
		super({
			...
			f1: true,
			// Full (not toggled) shows the «many knobs» icon; simplified (toggled) shows the minimal one.
			icon: iconFull,
			toggled: {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const cur = configurationService.getValue<boolean>(CONFIG_SIMPLIFIED_CONTROLS) ?? false;
