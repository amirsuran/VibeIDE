...
## [agent] Итерационный workflow: `npm run compile && run-dev.bat` (НЕ watch)
...
**Суть:**
- **Правильный цикл:** `npm run compile` (~1-3 мин, инкрементальный gulp compile) → `.\run-dev.bat` → проверка в Electron-окне с обновлёнными `out/`.
