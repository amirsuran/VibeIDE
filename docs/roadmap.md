...
- [ ] **ПРОВЕРИТЬ (unverified, аудит `wf_365c8028-33b`): `addAssistantNotice` во время активного LLM-стрима в том же треде.** Заявлено, что вставка display-only assistant-сообщения (чат-события субагентов + отчёт маршрута) при live-стриме основного агента может ломать инвариант «последнего сообщения» треда. Не доверифицировано (36 верификаторов упали по session-лимиту), не воспроизведено. Следующая сессия: во время активного ответа агента запустить роль → убедиться, что notice не перетирает/не рвёт стрим; при риске — буферизовать notice до конца стрима (`streamState`). Полный сырой отчёт аудита: OneDrive `.../subagents/workflows/wf_365c8028-33b`.
...
- [ ] **Живой индикатор «роль думает» В ОКНЕ ЧАТА (killer-запрос).** Сейчас: уведомления только `🧩 старт` / `✅ финиш`, а «крутилка» активной работы — только в нижнем статус-баре (`vibeSubagentStatusBarContribution`), не в чате. Пользователь хочет видеть в треде, что роль **сейчас думает/работает** (спиннер/индикатор между старт и финиш). Идея: живое сообщение-плейсхолдер роли в треде, обновляемое на `onSubagentStatusChanged` (running → крутилка «роль X работает…», terminal → заменить финишным). Учесть [[выше]] риск `addAssistantNotice` при стриме. Файлы: `vibeSubagentChatNoticeContribution.ts` + React-рендер сообщений.
...
- [ ] **Расход субагента идёт в session-бюджет токенов (пользователь: «не правильно»).** Спенд роли учитывается в общем session token budget/списке лимитов (`vibeTokenBudgetService`); мы отвязали КВОТУ субагента от остатка сессии, но фактические токены всё равно капают в session-счётчик. Решить: (а) исключить субагентов из session-бюджета (свой отдельный учёт), либо (б) показывать раздельно «сессия / субагенты». Заодно проверить глюк отображения индикатора бюджета на смоуке: `43 332·4%` (похоже на битый формат числа/процента — возможно тот же per-token/per-1M или локаль-разделитель). Файлы: `vibeTokenBudgetService.ts`, индикатор бюджета в статус-баре.
