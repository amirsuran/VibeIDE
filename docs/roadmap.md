...
- [x] **Звено 5 — точка входа** — ✅ (2026-07-12, `next`, `825c16b9`) модальный фреймворк получил image-вход: `VibeModalOptions.imageInput` → `VibeModalSimple` рендерит скрепку «📎 Картинка» + drag-drop + paste на поле + чипы вложений (переиспользует `useImageAttachments`); `showModal` возвращает `images` (`resolveHead`/`VibeModalResult.images`). Route-модалка включает `imageInput`, прокидывает картинки в `executeRoute`. Живой смоук: кнопка/копирайт рендерятся. Ограничение: вложения не переживают round-trip под-модалки «Роли» (документировано). **Vision-маршрутизация 1→5 замкнута.**
