...
- [ ] **Тумблер-форс C у инпута** — one-shot «🔎 Разведать» в тулбаре инпута форсит разведку любого запроса (не только continuation). `forceScout` через сервис уже проброшен; осталась UI-кнопка + арм-флаг (`_scoutNextTurnByThread` / `forceScout`).
