...
- [ ] **Звено 1 — image-aware маршрут** — при наличии картинки `buildRoute()` даёт ведущий vision-этап (роль-аналитик, дефолт `designer`), которому уходит изображение; текстовый вывод — по хендоффу кодерам.
- [ ] **Звено 3 — авто-vision-модель для роли-получателя** — при резолве модели vision-этапа: если выбранная/дефолтная модель роли не `supportsVision` → авто-фолбэк на vision-совместимую (+нотис), а не тихая потеря картинки.
- [ ] **Звено 4 — дизайнер vision-by-default** — пресет `designer` тянет дефолтную модель из vision-совместимых.
- [ ] **Звено 5 — точка входа** — поле «Промпт для субагента» в модалке маршрута: drag-n-drop, вставка из буфера, скрепка (переиспользовать механику основного инпута, тюнинг/размещение — на усмотрение); проброс приложенных в чате картинок в маршрут.
