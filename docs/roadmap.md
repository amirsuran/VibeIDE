...
- [ ] Модели на роль — через `modelSelectionOfFeature` (роль→предпочтительная модель): **остаётся** (сейчас роль наследует модель сессии). **⛔ ЗАБЛОКИРОВАНО Phase 3b (выяснено при разведке 2026-07-05):** `_runSubagent` в `vibeSubagentService.ts` — плейсхолдер («MVP: simulate a compact placeholder result», реальный tool-loop не выполняется, LLM не вызывается), `vibeSubagentIsolationRuntime` поднимает воркеры без LLM-цикла. Привязывать модель не к чему, пока не реализован real agent runner (headless tool-loop как у chatThreadService в изолированном контексте) — сначала Phase 3b, потом этот пункт.
