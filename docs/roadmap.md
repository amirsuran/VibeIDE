...
- [x] **D.2 — мягкий чекпоинт агентского цикла** — ✅ `vibeide.agent.softCheckpointIterations` (25) / `softCheckpointTokens` (1M), 0 = выкл; пауза с вопросом Продолжить/Остановить, независимо от жёсткого `maxLoopIterations`. *Открытое (D.2a):* sticky-промпт не резолвится при abort через кнопку стоп (только своими кнопками/закрытием) — завязать на interruptor. *Открытое (D.2b):* токен-чекпоинт фактически отключается после autopilot auto-reset (baseline становится больше текущего счётчика → `spent`=0); итерационный чекпоинт продолжает работать.
...
- [ ] **D.8 — pre-OOM user-нотификация (фича-кандидат)** — surface'ить `_onPreOomAlert` юзеру тостом с действием «Перезагрузить окно (освободить память)», раз renderer-OOM повторяется; `autoRestartOnPreOom` уже есть как авто-вариант.
