...
>
> ✅ **Windows-догон 1.7.0 ОПУБЛИКОВАН (2026-07-12).** Собран из ТОГО ЖЕ кода, что и mac — не с текущего main, а из тега: `git checkout v1.7.0` (detached HEAD) → Фаза 1 `release-windows.ps1 -Version v1.7.0 -SkipPublish` (bump-коммита нет — `product.json` уже 1.7.0) → по «гуд» Фаза 2 `-SkipCompile` (переупаковка из готового `out-build`, штамп сверен). Скрипт увидел существующий релиз тега (создан маком) и **долил** в него `VibeIDESetup.exe` + `VibeIDE-1.7.0-win32-x64.zip` — релиз стал кросс-платформенным (mac dmg/zip + Windows setup/portable). **Фаза 2b — winget:** PR [microsoft/winget-pkgs#401483](https://github.com/microsoft/winget-pkgs/pull/401483) (SHA256 с выложенного `.exe`, чек-лист авто-проставлен). Node 22 через fnm (`fnm env … | iex; fnm use 22` — дефолтный Node 24 иначе бьёт упаковку). Грабли и процедура догона — `docs/knowledge/build/windows-toolchain.md`.
