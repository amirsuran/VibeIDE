...
> ✅ **РЕЛИЗ 1.7.0 (2026-07-12) — merge `next`→main.** Фича-релиз: vision-маршрутизация ролей (картинка→дизайнер на vision-модели, авто-фолбэк, скрепка/дроп/пейст в маршруте), авто-разведчик на continuation-запросах (гейт «оно/не оно» в треде), упрощённый/полный вид панели чата (иконка-глаз, дефолт упрощённый), тултипы+перенос кнопок субагента/разведки к модели. 22 коммита с базлайна 1.6.0. What's New 1.7.0, README-бейдж, functional.md обновлены. Базлайн ченджлога → 1.7.0. Кросс-платформенно: mac Фаза 1 здесь, Windows догоняет той же версией.
