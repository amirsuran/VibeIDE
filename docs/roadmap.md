...
- [ ] **`vibeProviderProxyService` — мёртвый stub** (HTTP-interception «Phase 3b» не реализован, записей из send-пути нет → лог всегда пустой). Захват запроса теперь покрыт `promptDump` (renderer-side). Решить: либо удалить proxy-сервис+настройки `vibeide.debug.providerProxy.*`, либо реализовать (для response-capture). Сейчас вводит в заблуждение (настройка есть, не работает).
